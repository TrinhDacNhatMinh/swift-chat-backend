...
const getRedisOptions = (configService: ConfigService): RedisOptions => {
  const password = configService.get<string>(CONFIG_KEYS.REDIS_PASSWORD);
  const tlsEnabled = configService.get<boolean>(CONFIG_KEYS.REDIS_TLS);
  return {
    host: configService.getOrThrow<string>(CONFIG_KEYS.REDIS_HOST),
    port: configService.getOrThrow<number>(CONFIG_KEYS.REDIS_PORT),
    ...(password ? { password } : {}),
    ...(tlsEnabled ? { tls: {} } : {}),
  };
};
