...
export class MessageArchivingService {
  ...
  constructor(
    ...
    private messageArchiveModel: Model<MessageDocument>,
  ...
  async handleArchiving() {
    ...
    while (hasMore) {
      try {
        ...
        this.logger.log(
          `Archived batch of ${messagesToArchive.length} messages. Total so far: ${totalArchived}`,
        );
      } catch (error) {
    ...
    this.logger.log(
      `Message archiving process completed. Total archived: ${totalArchived}`,
    );
  }
}
