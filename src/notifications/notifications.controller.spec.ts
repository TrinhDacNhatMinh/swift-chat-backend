...
describe('NotificationsController', () => {
  ...
  it('should pass user.id and parsed limit to service.getUserNotifications when getNotifications() is called', async () => {
    ...
    expect(service.getUserNotifications).toHaveBeenCalledWith('u1', 10, 0);
  });
  ...
  it('should use default limit 20 to service.getUserNotifications when limit is not provided', async () => {
    ...
    expect(service.getUserNotifications).toHaveBeenCalledWith('u1', 20, 0);
  });
  ...
  it('should pass user.id to service.markAllAsRead when markAllAsRead() is called', async () => {
    ...
    const result = await controller.markAllAsRead({ id: 'u1' });
