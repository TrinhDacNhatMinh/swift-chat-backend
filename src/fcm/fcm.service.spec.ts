...
describe('FcmService', () => {
  ...
  describe('registerDevice()', () => {
    it('should upsert device token when registerDevice() is called', async () => {
      ...
      expect(prisma.deviceToken.upsert).toHaveBeenCalledWith({
        where: { token: 'tok' },
        update: { accountId: 'u1', platform: 'android' },
        create: { accountId: 'u1', token: 'tok', platform: 'android' },
      });
  ...
  describe('sendPushToUser()', () => {
    ...
    it('should return early when account has no devices in sendPushToUser()', async () => {
      (service as any).isInitialized = true;
      prisma.deviceToken.findMany.mockResolvedValue([]);
  ...
  describe('sendPushToOfflineParticipants()', () => {
    ...
    it('should filter offline users via Redis mget and send push when sendPushToOfflineParticipants() is called', async () => {
      ...
      prisma.participant.findMany.mockResolvedValue([
        { accountId: 'u2', mutedUntil: null },
        { accountId: 'u3', mutedUntil: null },
      ]);
