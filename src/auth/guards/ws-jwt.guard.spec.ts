...
describe('WsJwtGuard', () => {
  ...
  it('should return true and skip verification when accountId is already set in socket.data', () => {
    const socket = mockSocket({ data: { accountId: 'u1' } });
    const result = guard.canActivate(mockContext(socket));
    expect(result).toBe(true);
    expect(jwtService.verify).not.toHaveBeenCalled();
  });

  it('should verify token from handshake.auth.token and set accountId when token is present', () => {
    ...
    expect(result).toBe(true);
    expect(socket.data.accountId).toBe('u1');
  ...
  it('should verify token from Authorization header when handshake.auth.token is absent', () => {
    ...
    expect(socket.data.accountId).toBe('u2');
  });
