...
describe('User (e2e)', () => {
  ...
  describe('PATCH /api/v1/users/me', () => {
    it('should update own profile', async () => {
      const res = await request(app.getHttpServer())
        ...
        .set(authHeader(user.accessToken))
        .send({ handle: 'testuser_user1_updated' })
        .expect(200);

      expect(res.body.handle).toBe('testuser_user1_updated');
      // Sensitive fields MUST NOT be returned
      expect(res.body).not.toHaveProperty('passwordHash');
    });
  ...
  describe('GET /api/v1/users', () => {
    it('should return matching users for a search query', async () => {
      ...
      expect(found).toHaveProperty('handle');
    ...
    it('should return an empty array when no match', async () => {
      const res = await request(app.getHttpServer())
        .get('/api/v1/users/search')
        .query({ q: 'zzznobodymatchesthisxyz' })
        .set(authHeader(user.accessToken))
        .expect(200);
