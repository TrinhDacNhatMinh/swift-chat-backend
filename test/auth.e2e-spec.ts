...
describe('Auth (e2e)', () => {
  ...
  describe('POST /api/v1/auth/register', () => {
    it('should register a new user and return tokens', async () => {
      ...
      expect(res.body.data.account).toMatchObject({ email: validUser.email });
      // Sensitive fields MUST NOT be exposed
      expect(res.body.data.account).not.toHaveProperty('passwordHash');
      expect(res.body.data.account).not.toHaveProperty('providerId');
      expect(res.body.data.account).not.toHaveProperty('authProvider');
    });
    ...
    it('should return 400 when email is already in use', async () => {
      ...
      const hasProvider = await prisma.authProvider.findFirst({
        where: { accountId: acct!.id, provider: 'password' },
      });
      if (!hasProvider)
        await prisma.authProvider.create({
          data: { accountId: acct!.id, provider: 'password' },
        });
  ...
  describe('POST /api/v1/auth/login', () => {
    ...
    it('should login with correct credentials and return tokens', async () => {
      ...
      expect(res.body.account).toMatchObject({ email: validUser.email });
      // Sensitive fields MUST NOT be exposed
      expect(res.body.account).not.toHaveProperty('passwordHash');
      expect(res.body.account).not.toHaveProperty('providerId');
      expect(res.body.account).not.toHaveProperty('authProvider');
    });
