...
describe('Chat WebSocket (e2e)', () => {
  ...
  describe('chat:send_message', () => {
    it('should persist message and broadcast chat:receive_message to all room members', async () => {
      ...
      expect(incoming.senderId).toBe(sender.userId);
      expect(incoming.isDeleted).toBeFalsy();
    });
    ...
    it('should be visible via GET /messages after being sent over WebSocket', async () => {
      ...
      expect(found.senderId).toBe(sender.userId);
  ...
  describe('chat:unsend_message', () => {
    it('should soft-delete own message and broadcast chat:message_unsent to room', async () => {
      const { senderSock, receiverSock } = await setupRoom();
      ...
      const ack = await emitAck<any>(senderSock, 'chat:unsend_message', {
        conversationId,
        messageId,
      });

      expect(ack).toMatchObject({ status: 'unsent', messageId });

      const broadcast = await deletedPromise;
      expect(broadcast.messageId).toBe(messageId);
      expect(broadcast.conversationId).toBe(conversationId);
      expect(broadcast.unsentBy).toBe(sender.userId);
    });
  });
  ...
  describe('Permission — outsider', () => {
    ...
    it('should return error when outsider tries to unsend_message in a conversation they do not belong to', async () => {
      ...
      const ack = await emitAck<any>(outsiderSock, 'chat:unsend_message', {
        conversationId,
        messageId,
      });
  ...
  describe('chat:mark_read', () => {
    it('should mark message as read and emit chat:read_receipt to other room members', async () => {
      ...
      expect(receipt.accountId).toBe(receiver.userId);
  ...
  describe('chat:typing / chat:stop_typing', () => {
    it('should broadcast chat:user_typing to other room members', async () => {
      ...
      expect(typing.accountId).toBe(sender.userId);
    ...
    it('should broadcast chat:user_stop_typing to other room members', async () => {
      ...
      expect(stopTyping.accountId).toBe(sender.userId);
    });
  });
