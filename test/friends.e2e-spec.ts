...
describe('Friends (e2e)', () => {
  ...
  describe('POST /api/v1/friend-requests', () => {
    it('should send a friend request successfully', async () => {
      ...
      expect(res.body.friendRequest).toHaveProperty('id');
      expect(res.body.friendRequest).toHaveProperty('senderId', userA.userId);
      expect(res.body.friendRequest).toHaveProperty('receiverId', userB.userId);
  ...
  describe('Re-send friend request after rejection', () => {
    it('should not allow B to re-send a request to C immediately after C has rejected it (cooldown)', async () => {
      // userC rejected userB earlier.
      // The service has a 24-hour cooldown, so a second POST should fail with 400.
      const res = await request(app.getHttpServer())
        .post('/api/v1/friend-requests')
        .set(authHeader(userB.accessToken))
        .send({ receiverId: userC.userId })
